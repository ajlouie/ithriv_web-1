...
export class CommonsApiService {
    ...
    if (dataset.can_delete_data) {
      return this.http
        .delete<any>(
          ...
          `/commonssecure/data/datasets/file/${dataset.id}`, {
            headers: {},
          })
        .pipe(catchError(this.handleError));
    }
  ...
  restoreDatasetData(dataset: Dataset, user: User): Observable<any> {
    if (dataset.can_restore_data) {
      return this.http
        .put<any>(
          this.getLandingServiceUrl(user) +
          `/commonssecure/data/datasets/file/${dataset.id}/restore`, {},
          {
            headers: { EMAIL: user.eppn},
          }
        )
        .pipe(catchError(this.handleError));
    }
  ...
  deleteUserProjectPermission(
    ...
    return this.http
      .delete<any>(
        this.getLandingServiceUrl(user) +
        `/commonssecure/permissions/projects/users/` +
        project.id +
        `/` +
  ...
  deleteUserDatasetPermission(
    ...
    return this.http
      .delete<any>(
        this.getLandingServiceUrl(user) +
        `/commonssecure/permissions/datasets/users/` +
        dataset.id +
        `/` +
        userPermission.user_role +
        `/` +
  ...
  getDatasetFileVersions(
    ...
    return this.http
      .get<DatasetFileVersion[]>(
        this.getLandingServiceUrl(user) +
        `/commonssecure/data/datasets/file/` +
        dataset.id +
        `/versions`,
        {
          headers: {},
  ...
  getDatasetIrbInvestigators(user: User, dataset: Dataset): Observable<IrbInvestigator[]> {
    return this.http
      .get<IrbInvestigator[]>(
        this.getLandingServiceUrl(user) +
        `/commonssecure/permissions/datasets/investigators/` +
        dataset.id,
        {
          headers: {},
          responseType: 'json',
  ...
  getUserIrbNumbers(user: User): Observable<IrbNumber[]> {
    return this.http
      .get<IrbNumber[]>(
        this.getLandingServiceUrl(user) +
        `/commonssecure/permissions/user/irb_protocols`,
        {
          headers: {},
          responseType: 'json',
        }
